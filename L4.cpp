...
void l4_packet::parse_packet() {
    // Format: index|src_port|dest_port|data_bytes
    ...
    // src_port
    try {
        src_port = static_cast<uint16_t>(std::stoi(fields[1]));
    } catch (...) {
        std::cerr << "Error: Invalid src_port: " << fields[1] << std::endl;
        return;
    }
    // dst_port
    try {
        dst_port = static_cast<uint16_t>(std::stoi(fields[2]));
    } catch (...) {
        std::cerr << "Error: Invalid dst_port: " << fields[2] << std::endl;
        return;
    }
    // data
    data = fields[3];
...
bool l4_packet::validate_packet(open_port_vec open_ports,
    ...
    // Check if there's a matching open port
...
bool l4_packet::as_string(std::string &packet) {
    packet = "0|" + std::to_string(src_port) + "|" + std::to_string(dst_port) + "|" + data;
    return true;
} 